...
resource "azurerm_windows_virtual_machine" "api" {
  count = var.api_count
  name = "${var.datacenter}${var.environment}api${count.index+1}${random_id.api[count.index].hex}"
  ...
  availability_set_id = azurerm_availability_set.pmx_api.id

  network_interface_ids = [ 
    azurerm_network_interface.api[count.index].id
  ]

  # computer_name = replace("${var.datacenter}api${random_id.api[count.index].hex}${[count.index]}", "/[_\\-]/", "")
  computer_name = "${var.datacenter}${var.environment}api${count.index+1}${random_id.api[count.index].hex}"

  os_disk {
    name                 = "${var.datacenter}${var.environment}${random_id.api[count.index].hex}-os"
...
# resource "azurerm_managed_disk" "api" {
#   count = var.api_count
#   name = "${azurerm_windows_virtual_machine.api[count.index].name}-data"
#   location = azurerm_resource_group.pmxapp_rg.location
#   resource_group_name = azurerm_resource_group.pmxapp_rg.name
...
# resource "azurerm_virtual_machine_data_disk_attachment" "api" {
#   count = var.api_count
#   managed_disk_id = azurerm_managed_disk.api[count.index].id
#   virtual_machine_id = azurerm_windows_virtual_machine.api[count.index].id
#   lun = 0
#   caching = "ReadWrite"
# }
# #                                                              #
...
# resource "azurerm_virtual_machine_extension" "api_cs" {
#   count                = var.api_count
#   name                 = "${azurerm_windows_virtual_machine.api[count.index].name}-cs"
#   virtual_machine_id   = azurerm_windows_virtual_machine.api[count.index].id
...
#   {
#     "commandToExecute": "powershell -command \"${local.attachdisk_script}\" && powershell -ExecutionPolicy Unrestricted -File attachdisk.ps1 && powershell -command \"${local.adoagent_script}\" && powershell -ExecutionPolicy Unrestricted -File az_agent_windows.ps1 ${local.api_alias_tag} ${data.azurerm_key_vault_secret.environmentSecret.value}"
#   }
# PROTECTEDSETTINGS
...
#   depends_on = [
#     azurerm_virtual_machine_data_disk_attachment.api
#   ]
# }
...
#   depends_on = [
#     azurerm_virtual_machine_extension.api_jd
#   ]
# }

resource "azurerm_virtual_machine_extension" "api_cs" {
  count                = var.azfileshare == true ? var.api_count : 0
  name                 = "${azurerm_windows_virtual_machine.api[count.index].name}-cs"
  virtual_machine_id   = azurerm_windows_virtual_machine.api[count.index].id
  publisher            = "Microsoft.Compute"
  type                 = "CustomScriptExtension"
...
resource "azurerm_virtual_machine_extension" "api_jd" {
  count                = var.api_count
  name                 = "DomainJoin"
  virtual_machine_id   = azurerm_windows_virtual_machine.api[count.index].id
  publisher            = "Microsoft.Compute"
...
#   PROTECTED_SETTINGS
  ...
  depends_on = [
    azurerm_virtual_machine_extension.api_cs
  ]
}
...
#   application_security_group_id = module.security_groups.application_security_groups[local.application_security_groups.api]
# }
#                                                              #
