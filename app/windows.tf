...
resource "azurerm_windows_virtual_machine" "windows" {
  ...
  name                = "${var.datacenter}${var.environment}wi${count.index+1}${random_id.windows[count.index].hex}"
  ...
  availability_set_id = azurerm_availability_set.pmx_windows.id

  network_interface_ids = [
      azurerm_network_interface.windows[count.index].id
    ]

  computer_name = "${var.datacenter}${var.environment}wi${count.index+1}${random_id.windows[count.index].hex}"

  os_disk {
    name                 = "${var.datacenter}${random_id.windows[count.index].hex}-os"
...
#   count               = var.windows_count
#   name                = "${azurerm_windows_virtual_machine.windows[count.index].name}-data"
#   location            = azurerm_resource_group.pmxapp_rg.location
#   resource_group_name = azurerm_resource_group.pmxapp_rg.name
...
#   count = var.windows_count
#   managed_disk_id = azurerm_managed_disk.windows[count.index].id
...
# resource "azurerm_virtual_machine_extension" "windows_cs" {
#   count                = var.windows_count
#   name                 = "${azurerm_windows_virtual_machine.windows[count.index].name}-cs"
#   virtual_machine_id   = azurerm_windows_virtual_machine.windows[count.index].id
...
#   {
#     "commandToExecute": "powershell -command \"${local.attachdisk_script}\" && powershell -ExecutionPolicy Unrestricted -File attachdisk.ps1 && powershell -command \"${local.adoagent_script}\" && powershell -ExecutionPolicy Unrestricted -File az_agent_windows.ps1 ${local.wi_alias_tag} ${data.azurerm_key_vault_secret.environmentSecret.value}"
#   }
# PROTECTEDSETTINGS
...
#   depends_on = [
#     azurerm_virtual_machine_extension.windows_jd
#   ]
# }

resource "azurerm_virtual_machine_extension" "windows_cs" {
  count                = var.azfileshare == true ? var.windows_count : 0
  name                 = "${azurerm_windows_virtual_machine.windows[count.index].name}-cs"
  virtual_machine_id   = azurerm_windows_virtual_machine.windows[count.index].id
  publisher            = "Microsoft.Compute"
  type                 = "CustomScriptExtension"
...
resource "azurerm_virtual_machine_extension" "windows_jd" {
  count                = var.windows_count
  name                 = "DomainJoin"
  virtual_machine_id   = azurerm_windows_virtual_machine.windows[count.index].id
  publisher            = "Microsoft.Compute"
...
#   PROTECTED_SETTINGS
  ...
  depends_on = [
    azurerm_virtual_machine_extension.windows_cs
  ]
}
...
#   application_security_group_id = module.security_groups.application_security_groups[local.application_security_groups.windows]
# }
